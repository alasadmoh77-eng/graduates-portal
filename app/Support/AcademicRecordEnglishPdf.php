...
class AcademicRecordEnglishPdf
{
    ...
    /** @var array<string, string> */
    private static array $examSessions = [
        'يونيو' => 'June session',
        'سبتمبر' => 'September session',
        'يناير' => 'January session',
        'الدور الأول' => 'First session',
        'الدور الثاني' => 'Second session',
        'الأول' => 'First session',
        'الثاني' => 'Second session',
    ];
    ...
    public static function examSession(?string $value): string
    {
        if ($value === null) {
            return '—';
        }
        ...
        if (isset(self::$examSessions[$t])) {
            return self::$examSessions[$t];
        }
        foreach (self::$examSessions as $ar => $en) {
            if (str_contains($t, $ar)) {
                return $en;
            }
        }
        
        // Fallback: if it's already in English or not in dictionary, check if it contains 'session'
        if (preg_match('/session/i', $t)) {
            return $t;
        }
        return $t . ' session';
    }
