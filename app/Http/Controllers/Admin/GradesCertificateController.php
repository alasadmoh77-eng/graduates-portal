...
use App\Http\Requests\Admin\StoreGradesCertificateRequest;
use App\Models\User;
use App\Services\GradesCertificateStorageService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\RedirectResponse;
use Illuminate\View\View;

class GradesCertificateController extends Controller
{
    public function __construct(
        protected \App\Services\AcademicRecordStorageService $storageService
    ) {}
    ...
    public function edit(User $graduate): View
    {
        ...
        $latestRequest = \App\Models\DocumentRequest::where('user_id', $graduate->id)
            ->whereHas('documentType', function($q) {
                $q->where('code', 'GRADES_CERTIFICATE');
            })
            ->latest()
            ->first();

        return view('admin.grades-certificate.edit', [
            'graduate' => $graduate,
            'initialData' => $initialData,
            'saveUrl' => route('admin.graduates.grades-certificate.update', $graduate),
    ...
    public function update(StoreGradesCertificateRequest $request, User $graduate): RedirectResponse|JsonResponse
    {
        abort_unless($graduate->role === 'graduate' && $graduate->graduate !== null, 404);
        ...
        if ($request->wantsJson()) {
            return response()->json(['ok' => true, 'message' => __('app.grades_certificate_saved')]);
        }

        return redirect()
            ->route('admin.graduates.grades-certificate.edit', $graduate)
            ->with('success', __('app.grades_certificate_saved'));
    }
