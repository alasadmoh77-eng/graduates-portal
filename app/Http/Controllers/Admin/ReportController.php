...
class ReportController extends Controller
{
    ...
    {
        $query = User::where('role', 'graduate')->with('graduate.major.faculty');

        if ($request->filled('faculty_id')) {
            $query->whereHas('graduate.major', fn($q) => $q->where('faculty_id', $request->faculty_id));
        }
    ...
    public function exportGraduates(Request $request)
    {
        ...
        return response()->streamDownload(function() use ($query) {
            ...
            fputcsv($handle, ['الاسم', 'البريد الإلكتروني', 'الرقم الجامعي', 'الكلية', 'التخصص', 'سنة التخرج']);
            ...
            $query->chunk(100, function($rows) use ($handle) {
                foreach ($rows as $row) {
                    fputcsv($handle, [
                        ...
                        $row->graduate->major->faculty->name_ar ?? 'N/A',
