...
class GradesCertificate extends Model
{
    protected $table = 'grades_certificates';
    ...
    {
        return $this->hasMany(GradesCertificateLevel::class, 'grades_certificate_id')->orderBy('sort_order');
    }
    ...
    public function toAlpinePayload(): array
    {
        ...
        $levels = $this->levels->map(function (GradesCertificateLevel $level) {
            $semesters = $level->semesters->map(function (GradesCertificateSemester $sem) {
                $subjects = $sem->subjects->map(fn (GradesCertificateSubject $s) => [
        ...
        return [
            'student' => [
                ...
                'honors' => ($this->honors_rank === 'مع مرتبة الشرف') ? 'مستحق' : ($this->honors_rank ?? ''),
