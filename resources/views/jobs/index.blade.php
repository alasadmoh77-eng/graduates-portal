...
<div class="row">
    @forelse($jobs as $job)
        <div class="col-md-6 mb-4">
            <div class="card h-100 border-0 shadow-sm hover-shadow transition {{ $job->is_filled ? 'job-card-filled' : '' }}">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        <div>
                            <span class="badge bg-success bg-opacity-10 text-success px-3 py-2 rounded-pill">{{ $job->job_type }}</span>
                            @if($job->is_filled)
                                <span class="badge bg-danger bg-opacity-10 text-danger px-3 py-2 rounded-pill ms-1">{{ app()->getLocale() === 'ar' ? 'تم شغلها' : 'Filled' }}</span>
                            @else
                                <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-2 rounded-pill ms-1">{{ app()->getLocale() === 'ar' ? 'متاحة' : 'Available' }}</span>
                            @endif
                        </div>
                    ...
                    <div class="d-flex align-items-center mb-4">
                        ...
                        <div class="ms-3">
                            <h6 class="mb-0 fw-bold">{{ $job->company?->company_name ?? $job->employer->name }}</h6>
                    ...
                    <div class="d-grid gap-2">
                        <a href="{{ route('graduate.jobs.show', $job) }}" class="btn btn-outline-primary rounded-pill py-2 fw-bold">
                            @if($job->is_filled)
                                {{ app()->getLocale() === 'ar' ? 'التفاصيل (غير متاحة للتقديم)' : 'Details (Unavailable)' }}
                            @else
                            ...
                            <div class="modal-content border-0">
                                <form action="{{ route('graduate.jobs.apply', $job->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        @if($job->is_filled)
                                            <button type="button" class="btn btn-secondary px-4 btn-apply-disabled" disabled>
                                                {{ app()->getLocale() === 'ar' ? 'غير متاحة للتقديم' : 'Unavailable' }}
                                            </button>
                                        @else
