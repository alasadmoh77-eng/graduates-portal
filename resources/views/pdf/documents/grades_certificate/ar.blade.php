        ...
        <table class="masthead-body">
            <tr>
                <td class="masthead-main">
                    <div class="doc-title-inline">{{ ar('شهادة درجات و تقديرات') }}</div>
                    
                    <div class="official-intro" style="font-size: 7.5px; line-height: 1.5; text-align: right; margin-top: 6px; padding-inline-end: 15px;">
                        {{ ar('تشهد جامعة إقليم سبأ كلية علوم الحاسوب والمعلومات أن الطالب /') }} <strong>{{ ar($arName) }}</strong>
                        <br>
                        {{ ar('رقم القيد:') }} <strong><span style="font-family: monospace;">{{ $arUid }}</span></strong>
                        <br>
                        {{ ar('للعام الجامعي:') }} <strong>{{ ar($arEnrollY) }}</strong>
                        <br><br>
                        {{ ar('وقد حصل على درجة بكالوريوس في تخصص') }} <strong>{{ ar($arMajor) }}</strong>
                        <br>
                        {{ ar('في دور') }} <strong>{{ ar($arExamSession) }}</strong> {{ ar('للعام') }} <strong>{{ ar($arGradY) }}</strong>{{ ar('م') }}
                        <br><br>
                        {{ ar('بتقدير:') }} <strong>{{ ar($arRating) }}</strong>
                        <br>
                        {{ ar('وبمعدل:') }} <strong>{{ ar($arGpa) }}%</strong>
                    </div>
                </td>
