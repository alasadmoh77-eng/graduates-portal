...
@section('title', __('app.edit_faculty'))
    ...
    <div class="col-lg-8">
        ...
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{ __('app.edit_faculty') }}</li>
            </ol>
        </nav>
        ...
        <div class="form-card p-4 p-md-5">
            ...
            <div class="d-flex align-items-center gap-3 mb-4">
                ...
                <div>
                    <h4 class="fw-bold mb-0 text-dark">{{ __('app.edit_faculty') }}</h4>
                    <p class="text-muted small mb-0">{{ __('app.edit_faculty_hint') }}</p>
                </div>
            </div>

            <div class="section-divider"></div>

            <form action="{{ route('admin.faculties.update', $faculty) }}" method="POST" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4"
                        ...
                        placeholder="{{ app()->getLocale() == 'ar' ? 'اكتب وصفاً للكلية (اختياري)' : 'Write a description for the faculty (optional)' }}">{{ old('description', $faculty->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                        <option value="active" {{ old('status', $faculty->status) === 'active' ? 'selected' : '' }}>{{ __('app.active') }}</option>
                        <option value="inactive" {{ old('status', $faculty->status) === 'inactive' ? 'selected' : '' }}>{{ __('app.inactive') }}</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
