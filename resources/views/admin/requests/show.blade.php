...
@php
    $statusThemes = [
        ...
        'PENDING_SIGNATURES' => ['bg' => 'bg-orange bg-opacity-10 text-warning', 'border' => 'border-warning', 'icon' => 'fa-file-signature'],
...
<div class="container-fluid py-4 px-lg-5">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            @if($documentRequest->payment_status !== 'not_required')
                <div class="admin-card mb-4 border-warning border-opacity-50">
                    <div class="p-4">
                        <h6 class="fw-bold mb-3"><i class="fas fa-credit-card text-warning me-2"></i> {{ __('app.payment_status') }}</h6>
                        <div class="row g-3">
                            <div class="col-md-3">
                                <div class="info-label">{{ __('app.fee_amount') }}</div>
                                <div class="info-value">{{ number_format($documentRequest->fee_amount, 0) }} {{ $documentRequest->currency }}</div>
                            </div>
                            <div class="col-md-3">
                                <div class="info-label">{{ __('app.payment_status') }}</div>
                                <div>
                                    @php
                                        $psBadge = ['pending_review' => 'bg-warning text-dark', 'approved' => 'bg-success', 'rejected' => 'bg-danger'];
                                        $psLabel = ['pending_review' => __('app.payment_pending_review'), 'approved' => __('app.payment_approved'), 'rejected' => __('app.payment_rejected')];
                                    @endphp
                                    <span class="badge {{ $psBadge[$documentRequest->payment_status] ?? 'bg-secondary' }} rounded-pill">{{ $psLabel[$documentRequest->payment_status] ?? $documentRequest->payment_status }}</span>
                                </div>
                            </div>
                            @if($documentRequest->payment_reviewed_by)
                            <div class="col-md-3">
                                <div class="info-label">{{ __('app.reviewed_by') }}</div>
                                <div class="info-value small">{{ $documentRequest->paymentReviewedBy?->name ?? '--' }}</div>
                            </div>
                            @endif
                            @if($documentRequest->payment_reviewed_at)
                            <div class="col-md-3">
                                <div class="info-label">{{ __('app.reviewed_at') }}</div>
                                <div class="info-value small">{{ $documentRequest->payment_reviewed_at->format('Y-m-d H:i') }}</div>
                            </div>
                            @endif
                        </div>
                        @if($documentRequest->payment_status === 'rejected' && $documentRequest->payment_rejection_reason)
                        ...
                        @if($documentRequest->payment_proof_path)
                            <div class="mt-2">
                                <a href="{{ route('admin.payments.proof', $documentRequest) }}" class="btn btn-sm btn-outline-info rounded-pill" target="_blank">
                                    <i class="fas fa-eye me-1"></i> {{ __('app.view_proof') }}
                                </a>
                            </div>
                        @endif
                    </div>
        ...
        <div class="col-lg-4">
                ...
                <div class="admin-card mb-4 border-warning border-opacity-50">
                    <div class="admin-card-header bg-warning bg-opacity-10">
                        <h5 class="fw-bold text-dark mb-0">
                            ...
                            @if($documentRequest->documentType->code === 'ACADEMIC_RECORD')
                                إدارة السجل الأكاديمي
                            @else
                                إدارة شهادة الدرجات والتقديرات
                            @endif
                        </h5>
                    </div>
                    <div class="p-4 bg-white">
                        @if(!($hasAcademicRecordData ?? true))
                            <p class="small text-danger fw-bold mb-2">
                                @if($documentRequest->documentType->code === 'ACADEMIC_RECORD')
                                    {{ __('app.academic_record_missing_title') }}
                                @else
                                    {{ __('app.grades_certificate_missing_title') }}
                                @endif
                            </p>
                            <p class="small text-muted mb-3">
                                @if($documentRequest->documentType->code === 'ACADEMIC_RECORD')
                                    {{ __('app.academic_record_missing_hint') }}
                                @else
                                    {{ __('app.grades_certificate_missing_hint') }}
                                @endif
                            </p>
                            @if($documentRequest->documentType->code === 'ACADEMIC_RECORD')
                            ...
                            @else
                                @can('edit-academic-record')
                                <a href="{{ route('admin.graduates.grades-certificate.edit', $documentRequest->user) }}" class="btn btn-warning rounded-pill fw-bold w-100 mb-2">
                                    <i class="fas fa-plus-circle me-2"></i> إدخال شهادة الدرجات والتقديرات
                                </a>
                                @endcan
                            @endif
                        @else
                            ...
                            <p class="small text-muted mb-3">
                                @if($documentRequest->documentType->code === 'ACADEMIC_RECORD')
                                    إذا وجدت خطأ في البيانات، عدّل السجل الأكاديمي ثم أعد إنشاء الوثيقة من هذه الصفحة.
                                @else
                                    إذا وجدت خطأ في البيانات، عدّل شهادة الدرجات ثم أعد إنشاء الوثيقة من هذه الصفحة.
                                @endif
                            </p>
                            <div class="d-flex flex-column gap-2 mb-3">
                                <span class="badge bg-light text-dark border text-start p-2"><i class="fas fa-edit text-primary me-2"></i>1. {{ $documentRequest->documentType->code === 'GRADES_CERTIFICATE' ? 'تعديل شهادة الدرجات وحفظها' : 'تعديل السجل الأكاديمي وحفظه' }}</span>
                            ...
                            @can('edit-academic-record')
                            <a href="{{ $documentRequest->documentType->code === 'GRADES_CERTIFICATE' ? route('admin.graduates.grades-certificate.edit', $documentRequest->user) : route('admin.graduates.academic-record.edit', $documentRequest->user) }}" class="btn btn-outline-warning text-dark border-warning rounded-pill fw-bold w-100">
                                <i class="fas fa-edit me-2"></i> {{ $documentRequest->documentType->code === 'GRADES_CERTIFICATE' ? 'تعديل شهادة الدرجات والتقديرات' : 'تعديل السجل الأكاديمي' }}
                            </a>
                            @endcan
                        @endif
                    </div>
            ...
            @if(in_array($documentRequest->status, ['APPROVED', 'READY', 'ISSUED'], true))
            <div class="admin-card mb-4 border-success border-opacity-25">
                ...
                <div class="p-4 bg-white text-center">
                    ...
                    @php
                        ...
                        $issuedDoc = $documentRequest->issuedDocument;
                        $allSigned = $issuedDoc && $issuedDoc->all_signed_at;
                    @endphp
                    ...
                    @if($documentRequest->status === 'APPROVED')
                        ...
                        <form action="{{ route('admin.requests.send-for-signatures', $documentRequest) }}" method="POST" class="mb-3">
                            @csrf
                            <button type="submit" class="btn btn-primary btn-lg w-100 py-3 rounded-pill shadow-sm fw-bold" @if(!$canGenerateAcademicPdf) disabled @endif>
                        ...
                        @php
                            ...
                            $isGrades = in_array($docCode, ['GRADES_CERTIFICATE', 'grade_certificate']);
                            $signers = $isGrades 
                                ? ['مسجل الكلية', 'عميد الكلية', 'المسجل العام', 'نائب رئيس الجامعة لشؤون الطلاب']
                                : ['المختص الأكاديمي', 'مدير إدارة شؤون الخريجين', 'مسجل الكلية', 'عميد الكلية'];
                        @endphp
                        <div class="small text-muted mt-2">
                            <i class="fas fa-users me-1"></i> سيتم إرسالها للموقعين:
                            <span class="fw-bold text-dark">{{ implode('، ', $signers) }}</span>
                        </div>
                    @endif

                    {{-- PENDING_SIGNATURES + all signed: show Approve & Issue button --}}
                    @if($documentRequest->status === 'PENDING_SIGNATURES' && $allSigned)
                        <p class="fw-bold text-success mb-3">جميع التوقيعات مكتملة! اعتمد الوثيقة وأصدرها:</p>
                        
                        <form action="{{ route('admin.documents.approve-issue', $issuedDoc) }}" method="POST" class="mb-3">
                            @csrf
                            <button type="submit" class="btn btn-success btn-lg w-100 py-3 rounded-pill shadow-sm fw-bold">
                                <i class="fas fa-check-circle me-2"></i> اعتماد وإصدار الوثيقة
                            </button>
                        </form>
                    @endif

                    {{-- PENDING_SIGNATURES + NOT all signed: show status --}}
                    @if($documentRequest->status === 'PENDING_SIGNATURES' && !$allSigned)
                        <div class="text-center py-3">
                            <i class="fas fa-clock fa-3x text-warning mb-3"></i>
                            <p class="fw-bold">الوثيقة بانتظار التوقيعات</p>
                            <p class="small text-muted mb-0">زر "اعتماد وإصدار" سيظهر بعد اكتمال جميع التوقيعات</p>
                        </div>
                    @endif
                    ...
                    @if(!$canGenerateAcademicPdf)
                        <p class="small text-danger mb-0 mt-2">
                            @if($documentRequest->documentType->code === 'ACADEMIC_RECORD')
                                {{ __('app.academic_record_missing_title') }} — {{ __('app.academic_record_open_entry') }}
                            @else
                                {{ __('app.grades_certificate_missing_title') }} — يرجى فتح صفحة الإدخال أولاً
                            @endif
                        </p>
                    @endif

                    @if($issuedDoc && $issuedDoc->pdf_path && $issuedDoc->issued_at)
                        <div class="border-top pt-3 mt-3 text-start">
                            ...
                            <div class="bg-light p-3 rounded-3 d-flex justify-content-between align-items-center">
                                <div>
                                    <div class="small fw-bold text-dark font-monospace mb-1">{{ $issuedDoc->serial_number }}</div>
                                    <div style="font-size:0.7rem;" class="text-muted">تم الانشاء: {{ $issuedDoc->issued_at->format('Y-m-d') }}</div>
                                </div>
                                <a href="{{ route('admin.requests.download-pdf', $documentRequest) }}" target="_blank" class="btn btn-sm btn-outline-primary" title="المعاينة والطباعة">
                    ...
                    @if($issuedDoc && in_array($documentRequest->status, ['READY', 'ISSUED']))
                        <div class="border-top pt-3 mt-3">
                            ...
                            <form action="{{ route('admin.documents.reissue', $issuedDoc) }}" method="POST"
                                  onsubmit="return confirm('هل أنت متأكد من إعادة الوثيقة لمرحلة التوقيعات؟ سيتم حذف جميع التوقيعات الحالية وإرسالها للتوقيع من جديد.')">
                                @csrf
                                <button type="submit" class="btn btn-warning rounded-pill px-4 fw-bold shadow-sm">
                                    <i class="fas fa-redo me-2"></i> إعادة إصدار الوثيقة
                                </button>
                            </form>
                        </div>
                    @endif
