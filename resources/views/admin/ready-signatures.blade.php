@extends('layouts.app')

@section('title', 'التوقيعات الجاهزة')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold mb-0">
            <i class="fas fa-check-circle text-success me-2"></i>التوقيعات الجاهزة
        </h2>
        <div>
            <a href="{{ route('admin.ready-signatures.export') }}" class="btn btn-success rounded-pill me-2">
                <i class="fas fa-file-excel me-1"></i> تصدير Excel
            </a>
            <a href="{{ route('admin.pending-signatures') }}" class="btn btn-outline-primary rounded-pill me-2">
                <i class="fas fa-clock me-1"></i> التوقيعات المعلقة
            </a>
            <a href="{{ route('admin.dashboard') }}" class="btn btn-outline-secondary rounded-pill">
                <i class="fas fa-arrow-right me-1"></i> لوحة التحكم
            </a>
        </div>
    </div>

    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show rounded-3">{{ session('success') }}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
    @endif

    {{-- Filters --}}
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-header bg-white border-bottom p-3">
            <h6 class="fw-bold mb-0 text-secondary"><i class="fas fa-filter me-2"></i> أدوات التصفية والبحث</h6>
        </div>
        <div class="card-body p-4">
            <form action="{{ route('admin.ready-signatures') }}" method="GET" class="row g-3">
                <div class="col-xl-3 col-lg-4 col-md-6">
                    <label class="form-label small fw-bold text-muted">البحث (الاسم، رقم الطلب)</label>
                    <div class="input-group">
                        <span class="input-group-text bg-light border-end-0"><i class="fas fa-search text-muted"></i></span>
                        <input type="text" name="search" class="form-control border-start-0 ps-0" value="{{ request('search') }}" placeholder="ابحث...">
                    </div>
                </div>
                <div class="col-xl-2 col-lg-4 col-md-6">
                    <label class="form-label small fw-bold text-muted">نوع الوثيقة</label>
                    <select name="document_type_id" class="form-select">
                        <option value="">كل الأنواع</option>
                        @foreach($types as $type)
                            <option value="{{ $type->id }}" {{ request('document_type_id') == $type->id ? 'selected' : '' }}>{{ $type->name_ar }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-xl-2 col-lg-3 col-md-4">
                    <label class="form-label small fw-bold text-muted">من تاريخ</label>
                    <input type="text" name="date_from" class="form-control date-picker-input" value="{{ request('date_from') }}" placeholder="YYYY-MM-DD" dir="ltr" lang="en" readonly>
                </div>
                <div class="col-xl-2 col-lg-3 col-md-4">
                    <label class="form-label small fw-bold text-muted">إلى تاريخ</label>
                    <input type="text" name="date_to" class="form-control date-picker-input" value="{{ request('date_to') }}" placeholder="YYYY-MM-DD" dir="ltr" lang="en" readonly>
                </div>
                <div class="col-xl-2 col-lg-2 col-md-4 d-flex align-items-end gap-2">
                    <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold"><i class="fas fa-filter me-1"></i> تصفية</button>
                    <a href="{{ route('admin.ready-signatures') }}" class="btn btn-outline-secondary rounded-pill px-3"><i class="fas fa-times"></i></a>
                </div>
            </form>
        </div>
    </div>

    @if($readyDocs->isEmpty())
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-body text-center py-5">
                <i class="fas fa-file-alt fa-4x text-muted mb-3"></i>
                <h5 class="fw-bold text-dark">لا توجد وثائق مكتملة التوقيع بعد</h5>
                <p class="text-muted">الوثائق التي تكتمل توقيعاتها ستظهر هنا تلقائياً.</p>
            </div>
        </div>
    @else
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white p-4 border-0">
                <h5 class="fw-bold mb-0">الوثائق مكتملة التوقيع ({{ $readyDocs->total() }})</h5>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead class="table-light">
                            <tr>
                                <th class="ps-4">#</th>
                                <th>رقم الطلب</th>
                                <th>الخريج</th>
                                <th>الرقم الجامعي</th>
                                <th>نوع الوثيقة</th>
                                <th>تاريخ الاكتمال</th>
                                <th>الموقعون</th>
                                <th>الحالة</th>
                                <th class="text-center">تحميل</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($readyDocs as $doc)
                                @php $request = $doc->documentRequest; @endphp
                                <tr>
                                    <td class="ps-4">{{ ($readyDocs->currentPage() - 1) * $readyDocs->perPage() + $loop->iteration }}</td>
                                    <td><span class="font-monospace small fw-bold">{{ $request->tracking_code ?? '—' }}</span></td>
                                    <td><strong>{{ $request->user->name ?? '—' }}</strong></td>
                                    <td><small class="text-muted">{{ $request->user->graduate->university_id ?? '—' }}</small></td>
                                    <td>
                                        <span class="badge bg-primary bg-opacity-10 text-primary">
                                            {{ app()->getLocale() == 'ar' ? ($request->documentType->name_ar ?? '—') : ($request->documentType->name_en ?? '—') }}
                                        </span>
                                    </td>
                                    <td style="direction: ltr; text-align: right;">{{ $doc->all_signed_at->format('Y-m-d') }}</td>
                                    <td>
                                        <div class="d-flex flex-wrap gap-1">
                                            @foreach($doc->signatures as $sig)
                                                <span class="badge bg-success bg-opacity-10 text-success small" title="{{ $sig->user->name ?? '' }}">
                                                    <i class="fas fa-check-circle me-1"></i>{{ $sig->role_title }}
                                                </span>
                                            @endforeach
                                        </div>
                                    </td>
                                    <td><span class="badge bg-info rounded-pill">{{ $request->status ?? '—' }}</span></td>
                                    <td class="text-center">
                                        @if($request->issuedDocument && $request->issuedDocument->pdf_path)
                                            <a href="{{ route('admin.requests.download-pdf', $request) }}" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill">
                                                <i class="fas fa-download me-1"></i> PDF
                                            </a>
                                        @else
                                            <span class="text-muted small">—</span>
                                        @endif
                                        @if(in_array($request->status, ['READY', 'ISSUED']))
                                            <form action="{{ route('admin.documents.reissue', $doc) }}" method="POST"
                                                  onsubmit="return confirm('هل أنت متأكد من إعادة الوثيقة لمرحلة التوقيعات؟ سيتم حذف جميع التوقيعات الحالية وإرسالها للتوقيع من جديد.')"
                                                  class="mt-1">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-outline-warning rounded-pill">
                                                    <i class="fas fa-redo me-1"></i> إعادة إصدار
                                                </button>
                                            </form>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            </div>
        </div>

        <div class="d-flex justify-content-center mt-4">
            {{ $readyDocs->appends(request()->query())->links() }}
        </div>
    @endif
</div>
@endsection
