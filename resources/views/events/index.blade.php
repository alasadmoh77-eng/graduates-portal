                    ...
                    @auth
                        ...
                        @else
                            <button class="btn btn-secondary w-100 rounded-pill fw-bold" disabled>
                                {{ app()->getLocale() == 'ar' ? 'التسجيل متاح للخريجين فقط' : 'Registration only for graduates' }}
                            </button>
                        @endif
                    @else
                        <a href="{{ route('login') }}" class="btn btn-outline-warning w-100 rounded-pill fw-bold text-decoration-none d-block text-center">
                            {{ app()->getLocale() == 'ar' ? 'سجل دخولك للتسجيل' : 'Log in to register' }}
                        </a>
                    @endauth
                </div>
            </div>
