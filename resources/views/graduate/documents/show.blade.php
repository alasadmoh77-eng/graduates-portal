...
@section('content')
    @php
        ...
        $pdfOk = $document->issuedDocument && \Illuminate\Support\Facades\Storage::disk('local')->exists($document->issuedDocument->pdf_path);
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                <div class="detail-card p-4 p-md-5 h-100">
                    ...
                    @if($document->payment_status !== 'not_required')
                        <div class="mt-4 p-4 rounded-3 border 
                            @if($document->payment_status === 'approved') payment-approved-box
                            @elseif($document->payment_status === 'rejected') border-danger bg-danger bg-opacity-5
                            @else border-warning bg-warning bg-opacity-5 @endif">
                            <h6 class="fw-bold mb-3"><i class="fas fa-credit-card me-2"></i> {{ __('app.payment_status') }}</h6>
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <div class="info-label">{{ __('app.fee_amount') }}</div>
                                    <div class="info-value">{{ number_format($document->fee_amount, 0) }} {{ $document->currency }}</div>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <div>
                                        @php
                                            $psBadge = [
                                                ...
                                                'approved' => 'payment-approved-badge',
                                                'rejected' => 'bg-danger',
                                                'pending_upload' => 'bg-secondary',
                                            ];
                                            $psLabel = [
                                                'pending_review' => __('app.payment_pending_review'),
                                                'approved' => __('app.payment_approved'),
                                                'rejected' => __('app.payment_rejected'),
                                                'pending_upload' => __('app.pending_upload'),
                                            ];
                                        @endphp
                                        <span class="badge {{ $psBadge[$document->payment_status] ?? 'bg-secondary' }} rounded-pill">
                                            {{ $psLabel[$document->payment_status] ?? $document->payment_status }}
                                        </span>
                                    </div>
                                </div>
                                @if($document->payment_reviewed_by)
                                    <div class="col-md-4">
                                        <div class="info-label">{{ __('app.reviewed_by') }}</div>
                                        <div class="info-value small">{{ $document->paymentReviewedBy?->name ?? '--' }}</div>
                                    </div>
                                @endif
                            </div>
                                @if($document->payment_proof_path && in_array($document->payment_status, ['pending_review', 'approved']))
                                    <div class="mt-3">
                                        <a href="{{ route('graduate.documents.view-proof', $document) }}" target="_blank"
                                           class="btn btn-outline-secondary btn-sm rounded-pill">
                                            <i class="fas fa-paperclip me-1"></i>
                                            {{ app()->getLocale() === 'ar' ? 'عرض إثبات الدفع المرفوع' : 'View Uploaded Payment Proof' }}
                                        </a>
                                    </div>
                                @endif
                                @if($document->payment_status === 'rejected' && $document->payment_rejection_reason)
                                <div class="alert alert-danger mt-3 mb-0 rounded-3">
                                    <strong><i class="fas fa-times-circle me-1"></i> {{ __('app.rejection_reason') }}:</strong>
                                    {{ $document->payment_rejection_reason }}
                                </div>
                                <div class="mt-3">
                                    <form action="{{ route('graduate.documents.payment-proof', $document) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <label class="form-label fw-bold small">{{ __('app.upload_payment_proof') }}</label>
                                        <div class="input-group">
                                            <input type="file" name="payment_proof" class="form-control" accept=".jpg,.jpeg,.png,.pdf" required>
                                            <button type="submit" class="btn btn-warning">{{ __('app.upload') }}</button>
                                        </div>
                                        <div class="small text-muted mt-1">{{ app()->getLocale() == 'ar' ? 'الصيغ المسموحة: JPG, JPEG, PNG, PDF - الحد الأقصى 5 ميجابايت.' : 'Allowed: JPG, JPEG, PNG, PDF - Max 5 MB.' }}</div>
                                    </form>
                                </div>
                            @endif
                        </div>
